...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QComboBox, QTextEdit, QFileDialog, QMessageBox, QListWidget, QListWidgetItem,
    QAbstractItemView, QDialog, QDialogButtonBox, QFormLayout, QCalendarWidget, QTabWidget
)
...
class BulletJournalApp(QMainWindow):
    ...
    def init_objetivos_tab(self):
        ...
        self.add_objetivo_button = QPushButton("Adicionar")
        self.add_objetivo_button.clicked.connect(self.add_objetivo)
        ...
        self.add_objetivo_button.setStyleSheet("background-color: #2196F3; color: white;")
        ...
        button_layout.addWidget(self.add_objetivo_button)
    ...
    def add_objetivo(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("Adicionar Objetivo")
        layout = QFormLayout(dialog)
        entry_input = QLineEdit()
        layout.addRow("Entrada:", entry_input)
        buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        ...
        if dialog.exec_() == QDialog.Accepted:
            new_entry = entry_input.text()
            if new_entry:
                objetivos_data["Objetivos Gerais"].append(new_entry)
                self.update_objetivos_list()
                QMessageBox.information(self, "Sucesso", "Objetivo adicionado com sucesso!")
            else:
    ...
    def add_to_entry(self):
        ...
        if selected_item:
            ...
            self.update_objetivos_list()
            QMessageBox.information(self, "Sucesso", "Objetivo adicionado à entrada com sucesso!")
        else:
            QMessageBox.warning(self, "Aviso", "Selecione um item para adicionar à entrada.")
